...
class Fuel_auth {
	...
	function front_login($user, $pwd)
	{
		$this->_CI->load->module_model(MEMBER_FOLDER, 'member_manage_model');
		
		$valid_user = $this->_CI->member_manage_model->valid_user($user, $pwd);
		if (!empty($valid_user))
		{
			//$valid_user = $this->_CI->users_model->user_info($valid_user['id']);
	...
	function front_is_logined()
	{
		$user = $this->valid_user();

		if(!empty($user) AND !empty($user['member_name']))
		{
			return true;
		}
		else if(!empty($user) AND !empty($user['user_name']))
		{
			return true;
		}
		else
		...
		//return (!empty($user) AND !empty($user['member_name']));		
	}
